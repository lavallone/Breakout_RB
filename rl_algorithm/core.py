...
class TrainEpisodeLogger(Callback):
    ...
    def on_episode_end(self, agent, **kwargs):
        ...
        template = '{step: ' + nb_step_digits + 'd}/{nb_steps}: episode: {episode}, duration: {duration: 5.3f}s, episode steps: {episode_steps: 5d}, episode reward: {episode_reward: 10.3f}'
        variables = {
            'step': self.step,
        ...
        ##################################################################
        ## WANDB LOGGING of cumulative reward
        wandb.log({"cumulative_reward" : np.sum(self.rewards[episode])})
        ##################################################################
